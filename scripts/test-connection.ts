...
import { InimClient } from '../src/inimClient';
import { InimWebSocket } from '../src/inimWebSocket';
...
async function main() {
  ...
  if (!user || !pass) {
    console.error(
      'Missing credentials. Pass them via --user / --pass or INIM_USER / INIM_PASS env vars.',
    );
    process.exit(2);
  }

  const log = new ConsoleLogger(verbose);
  const client = new InimClient({ username: user, password: pass, logger: log });
  ...
  for (const d of devices) {
    ...
    console.log('═══════════════════════════════════════════════════════');
    console.log(
      `Device  id=${d.DeviceId}  name="${d.Name}"  model=${d.ModelFamily ?? '?'} ${d.ModelNumber ?? ''}` +
    ...
    for (const a of d.Areas) {
      const armedLabel =
        a.Armed === 1
          ...
          : `partial(${a.Armed})`;
      console.log(
        `    - AreaId=${a.AreaId}  name="${a.Name}"  ${armedLabel}` +
          (a.Alarm ? '  !!! ALARM !!!' : ''),
      );
    }
    console.log('');
    ...
    for (const z of d.Zones) {
      ...
      const areasRaw = (z as { Areas?: unknown }).Areas;
      const areasStr = Array.isArray(areasRaw)
        ...
        : areasRaw == null
          ...
          : String(areasRaw);
      console.log(
        `    - ZoneId=${z.ZoneId}  name="${z.Name}"  ${stateLabel}` +
