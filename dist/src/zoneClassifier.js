...
function classifyZone(zoneName, mode = 'auto') {
    ...
    // auto
    if (matchesAny(zoneName, TAMPER_KEYWORDS))
        return 'skip';
    if (matchesAny(zoneName, MOTION_KEYWORDS))
        return 'motion';
    if (matchesAny(zoneName, DOOR_KEYWORDS))
        return 'contact';
    if (matchesAny(zoneName, WINDOW_KEYWORDS))
        return 'contact';
