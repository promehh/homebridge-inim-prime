...
exports.MotionZoneHandler = exports.ContactZoneHandler = exports.AreaSwitchHandler = exports.ScenarioSwitchHandler = exports.GlobalSecuritySystemHandler = exports.AreaSecuritySystemHandler = void 0;
// ---------- helpers --------------------------------------------------------
function readSecConstants(api) {
...
function isAlarming(area) {
    if (!area)
        return false;
    if (typeof area.Alarm === 'boolean')
        return area.Alarm;
    return !!area.Alarm;
}
function armedToHkCurrent(area, sec) {
    if (!area)
        return sec.DISARMED;
    if (isAlarming(area))
        return sec.ALARM_TRIGGERED;
    switch (area.Armed) {
        case 1:
            return sec.AWAY_ARM;
        case 2:
...
function ensureAccessoryInformation(acc, api, device) {
    const C = api.hap.Characteristic;
...
class AreaSecuritySystemHandler {
    constructor(env, accessory, deviceId, areaId) {
        this.env = env;
        ...
        if (device)
            ensureAccessoryInformation(accessory, api, device);
        this.service =
    ...
    async setTargetState(value) {
        ...
        this.env.logger.info(`[Area ${this.areaId}] Set target ${value} -> ${arm ? 'ARM' : 'DISARM'}`);
        try {
            await this.env.client.insertAreas(this.deviceId, [this.areaId], arm, this.env.config.userCode);
            ...
            this.env.logger.error(`[Area ${this.areaId}] Arm/disarm failed: ${e.message}`);
            throw new this.env.api.hap.HapStatusError(-70402 /* this.env.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE */);
        }
    }
    pushUpdate() {
        ...
        this.service.updateCharacteristic(C.SecuritySystemTargetState, cur === this.sec.ALARM_TRIGGERED ? this.sec.AWAY_ARM : cur);
    }
}
exports.AreaSecuritySystemHandler = AreaSecuritySystemHandler;
// ---------- GlobalSecuritySystemHandler -----------------------------------
class GlobalSecuritySystemHandler {
    constructor(env, accessory, deviceId) {
        this.env = env;
        this.accessory = accessory;
        this.deviceId = deviceId;
        const { api } = env;
    ...
    getCurrentState() {
        ...
        if (!device)
            return this.sec.DISARMED;
        // Alarm flag on any area => triggered.
        if (device.Areas.some(isAlarming))
            return this.sec.ALARM_TRIGGERED;
        ...
        if (typeof active === 'number') {
            if (sceneMap.disarmScenarioId === active)
                return this.sec.DISARMED;
            if (sceneMap.stayScenarioId === active)
                return this.sec.STAY_ARM;
            if (sceneMap.awayScenarioId === active)
                return this.sec.AWAY_ARM;
            if (sceneMap.nightScenarioId === active)
                return this.sec.NIGHT_ARM;
        }
        ...
        for (const a of device.Areas) {
            if (a.Armed === 1)
                armedCounts.full += 1;
            else if (a.Armed === 2 || a.Armed === 3)
                armedCounts.partial += 1;
            else
                armedCounts.disarmed += 1;
        }
        if (armedCounts.full === 0 && armedCounts.partial === 0)
            return this.sec.DISARMED;
        if (armedCounts.disarmed === 0 && armedCounts.partial === 0)
            return this.sec.AWAY_ARM;
        return this.sec.STAY_ARM;
    }
    getTargetState() {
    ...
    async setTargetState(value) {
        ...
        const sceneMap = this.env.config.sceneMapping ?? {};
        let scenarioId;
        if (v === this.sec.STAY_ARM)
            scenarioId = sceneMap.stayScenarioId;
        else if (v === this.sec.AWAY_ARM)
            scenarioId = sceneMap.awayScenarioId;
        else if (v === this.sec.NIGHT_ARM)
            scenarioId = sceneMap.nightScenarioId;
        else if (v === this.sec.DISARMED)
            scenarioId = sceneMap.disarmScenarioId;
        try {
            if (typeof scenarioId === 'number') {
                this.env.logger.info(`[Global] Set target ${v} -> ActivateScenario(${scenarioId})`);
                ...
                this.env.logger.info(`[Global] Set target ${v} -> InsertAreas(${arm ? 'arm' : 'disarm'} all ${areaIds.length})`);
                await this.env.client.insertAreas(this.deviceId, areaIds, arm, this.env.config.userCode);
                // Optimistic
                for (const a of device.Areas)
                    a.Armed = arm ? 1 : 4;
            }
            ...
            this.env.logger.error(`[Global] Set target failed: ${e.message}`);
            throw new this.env.api.hap.HapStatusError(-70402 /* this.env.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE */);
        }
    }
    pushUpdate() {
...
exports.GlobalSecuritySystemHandler = GlobalSecuritySystemHandler;
// ---------- ScenarioSwitchHandler -----------------------------------------
class ScenarioSwitchHandler {
    constructor(env, accessory, deviceId, scenarioId) {
        this.env = env;
        this.accessory = accessory;
        this.deviceId = deviceId;
    ...
    async setOn(v) {
        if (!v) {
            ...
            setTimeout(() => {
                this.service.updateCharacteristic(this.env.api.hap.Characteristic.On, this.isActive());
            }, 500);
            return;
        }
        try {
            ...
            const d = this.env.coordinator
                ...
                .find((dd) => dd.DeviceId === this.deviceId);
            if (d)
                d.ActiveScenario = this.scenarioId;
        }
        catch (e) {
            this.env.logger.error(`[Scenario ${this.scenarioId}] Activate failed: ${e.message}`);
            throw new this.env.api.hap.HapStatusError(-70402 /* this.env.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE */);
        }
    }
}
exports.ScenarioSwitchHandler = ScenarioSwitchHandler;
// ---------- AreaSwitchHandler ---------------------------------------------
class AreaSwitchHandler {
    constructor(env, accessory, deviceId, areaId) {
        this.env = env;
        this.accessory = accessory;
        this.deviceId = deviceId;
        ...
        env.coordinator.on('change', () => this.service.updateCharacteristic(C.On, this.isArmed()));
    }
    isArmed() {
    ...
    async setArmed(arm) {
        try {
            this.env.logger.info(`[Area ${this.areaId} switch] Set ${arm ? 'arm' : 'disarm'}`);
            await this.env.client.insertAreas(this.deviceId, [this.areaId], arm, this.env.config.userCode);
            const a = this.env.coordinator.findArea(this.deviceId, this.areaId);
            if (a)
                a.Armed = arm ? 1 : 4;
            ...
            this.env.logger.error(`[Area ${this.areaId} switch] Failed: ${e.message}`);
            throw new this.env.api.hap.HapStatusError(-70402 /* this.env.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE */);
        }
    }
}
exports.AreaSwitchHandler = AreaSwitchHandler;
// ---------- ContactZoneHandler --------------------------------------------
class ContactZoneHandler {
    constructor(env, accessory, deviceId, zoneId) {
        this.env = env;
        this.accessory = accessory;
        this.deviceId = deviceId;
        this.zoneId = zoneId;
        ...
        this.service
            ...
            .onGet(() => this.read());
        env.coordinator.on('change', () => this.service.updateCharacteristic(C.ContactSensorState, this.read()));
    }
    read() {
        const C = this.env.api.hap.Characteristic;
...
exports.ContactZoneHandler = ContactZoneHandler;
// ---------- MotionZoneHandler ---------------------------------------------
class MotionZoneHandler {
    constructor(env, accessory, deviceId, zoneId) {
        this.env = env;
        this.accessory = accessory;
        this.deviceId = deviceId;
        ...
        this.service
            ...
            .onGet(() => this.read());
        env.coordinator.on('change', () => this.service.updateCharacteristic(C.MotionDetected, this.read()));
    }
    read() {
        const z = this.env.coordinator.findZone(this.deviceId, this.zoneId);
        return (z?.Status ?? 1) === 2;
...
exports.MotionZoneHandler = MotionZoneHandler;
