...
function buildAck(seq, receiver, account) {
...
class SiaServer {
    ...
    async start() {
        return new Promise((resolve, reject) => {
            const server = (0, net_1.createServer)((socket) => this.handleClient(socket));
            this.server = server;
            server.once('error', reject);
            server.listen(this.opts.port, '0.0.0.0', () => {
                this.opts.logger.info(`SIA-IP listener on 0.0.0.0:${this.opts.port}` +
    ...
    handleClient(socket) {
        ...
        this.opts.logger.debug(`SIA-IP client connected: ${socket.remoteAddress}:${socket.remotePort}`);
        let buffer = '';
    ...
    processFrame(rawFrame, socket) {
        let frame = rawFrame;
        if (frame.startsWith('\n'))
            frame = frame.slice(1);
        if (frame.length < 8)
            return;
        if (!frame.includes('"SIA-DCS"'))
            return;
        ...
        try {
            ...
            this.opts.logger.debug(`ACK write failed: ${e.message}`);
        }
        if (this.opts.accountFilter && this.opts.accountFilter !== account) {
            this.opts.logger.debug(`SIA-IP frame ignored (account ${account} != ${this.opts.accountFilter})`);
            return;
        }
